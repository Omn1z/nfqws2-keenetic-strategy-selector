  ...
  return (
    <>
      ...
      <Card title="Сплит-маршрутизация" sub="как делить трафик между туннелем и прямым выходом">
        ...
        <p className="mt-0.5 text-[11px] text-muted">Маски (<b>*.com</b>, <b>*ip*</b>, <b>[re]…</b>) включают перехват DNS <b>автоматически</b> — отдельно тумблер для них не нужен. Тумблер дополнительно заводит в туннель <b>обычные</b> домены со всеми поддоменами и новыми IP. Перехват ловит только нешифрованный DNS через роутер; DoH/DoT на устройстве его обходит — для «всё и всегда» ставьте <b>*</b> или режим «Весь трафик».</p>
        <div className="mt-1 flex items-center gap-4"><Switch checked={!!r.sni_routing} onChange={(v) => setR({ ...r, sni_routing: v })} label="SNI-маршрутизация (CDN + DoH) — дополнительный слой" /></div>
        <p className="mt-0.5 text-[11px] text-muted">Читает имя сайта прямо из TLS-рукопожатия — <b>не зависит от DNS</b> (работает при DoH/DoT и за CDN типа Cloudflare). Домены из зон «Включить» заводятся в туннель по факту обращения и держатся ~1 ч. Первый коннект к новому адресу ещё уходит напрямую (по нему учимся), дальше — через VPN. Действует в режиме «по зонам».</p>
