...
const traceType = (e: TraceEntry) => {
  if (e.kind !== "flow") return e.qtype || "";
  const proto = e.proto === "udp" && e.dport === 443 ? "QUIC" : (e.proto || "").toUpperCase();
  const port = e.dport ? `/${e.dport}` : "";
  const state = e.state ? ` ${e.state}` : "";
  const event = e.event ? ` ${e.event}` : "";
  return `${proto}${port}${state}${event}`.trim();
};

const traceDst = (e: TraceEntry) => {
  if (!e.dst) return "";
  if (e.kind !== "flow" || !e.dport) return e.dst;
  return e.dst.includes(":") ? `[${e.dst}]:${e.dport}` : `${e.dst}:${e.dport}`;
};

const traceKindsLabel = (kinds: TraceKind[]) => {
  if (kinds.length === TRACE_KIND_OPTIONS.length) return "все";
  if (kinds.length === 0) return "ничего";
  const sel = new Set(kinds);
  return TRACE_KIND_OPTIONS.filter((o) => sel.has(o.value)).map((o) => o.label).join(", ");
};
...
export default function TracePane() {
  ...
  const [filterKinds, setFilterKinds] = useState<TraceKind[]>(ALL_TRACE_KINDS);
  ...
  const selectedKinds = useMemo(() => new Set(filterKinds), [filterKinds]);
  const toggleKind = (kind: TraceKind) => {
    setFilterKinds((prev) => {
      const next = prev.includes(kind) ? prev.filter((k) => k !== kind) : [...prev, kind];
      return TRACE_KIND_OPTIONS.filter((o) => next.includes(o.value)).map((o) => o.value);
    });
    setPage(1);
  };
  const setAllKinds = (on: boolean) => {
    setFilterKinds(on ? ALL_TRACE_KINDS : []);
    setPage(1);
  };
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      if (nm && !e.name.toLowerCase().includes(nm) && !traceDst(e).toLowerCase().includes(nm)) return false;
      if (tp && !traceType(e).toLowerCase().includes(tp)) return false;
      return true;
    });
  }, [entries, filterSrc, filterName, filterType, selectedKinds, filterDecision, ipToHost]);
  ...
  return (
    <Card>
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-line pb-3">
        <h2 className="text-[15px] font-semibold">Traffic Trace</h2>
      ...
      <div className="flex flex-wrap items-center gap-3 py-3">
        ...
        <details className="relative">
          <summary className="flex h-7 min-w-[116px] cursor-pointer list-none items-center justify-between gap-2 rounded border border-line bg-panel px-2 text-[12px] select-none [&::-webkit-details-marker]:hidden">
            <span>{traceKindsLabel(filterKinds)}</span>
            <span className="text-[10px] text-muted">▼</span>
          </summary>
          <div className="absolute left-0 z-20 mt-1 min-w-[140px] rounded border border-line bg-panel p-1 shadow-lg">
            <label className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-[12px] hover:bg-line-soft">
              <input
                type="checkbox"
                checked={filterKinds.length === TRACE_KIND_OPTIONS.length}
            ...
            {TRACE_KIND_OPTIONS.map((kind) => (
              <label key={kind.value} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-[12px] hover:bg-line-soft">
                <input
                  type="checkbox"
                  checked={selectedKinds.has(kind.value)}
        ...
        <table className="w-full text-[11px] md:text-[12px]">
          <thead className="bg-panel-soft text-[10px] uppercase text-muted md:text-[11px]">
            <tr>
              ...
              <Th onClick={() => onSort("name")}>Name{sortGlyph("name", sortKey, sortDir)}</Th>
            ...
            {slice.map((e, i) => {
              ...
              return (
                <tr key={`${e.ts}-${i}`} className={cn("border-t border-line", i % 2 ? "bg-panel-soft/40" : "")}>
                  ...
                  <td className="hidden whitespace-nowrap px-2 py-1 lg:table-cell">{traceType(e)}</td>
          ...
          <tfoot className="bg-panel-soft">
            <tr>
              ...
              <td className="px-2 py-1">
                <Input value={filterType} onChange={(e) => { setFilterType(e.target.value); setPage(1); }} placeholder="A/AAAA/tcp/udp/quic" className="h-7 text-[12px]" />
              </td>
              <td className="px-2 py-1">
                <Input value={filterName} onChange={(e) => { setFilterName(e.target.value); setPage(1); }} placeholder="Name / dst IP" className="h-7 text-[12px]" />
              </td>
              <td className="px-2 py-1">
