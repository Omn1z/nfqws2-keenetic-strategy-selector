...
export const fieldCls =
  "w-full min-w-0 max-w-full rounded-lg border border-line bg-input px-3 py-2 text-[13.5px] text-ink outline-none transition placeholder:text-muted focus:border-accent focus:ring-2 focus:ring-accent-w disabled:cursor-not-allowed disabled:opacity-50";

export function Field({ label, hint, className, children }: { label?: ReactNode; hint?: ReactNode; className?: string; children: ReactNode }) {
  return (
    <label className={cn("block min-w-0 max-w-full text-[13px] font-medium text-ink-soft", className)}>
      {label != null && (
