...
func awgFirewallHook(mode, endpointIP, wandev string, mtu int, dnsRedirect, chainEnabled, tunnelV6 bool, zones []awg.Zone) string {
	...
	s.WriteString("  while iptables -w 5 -t mangle -D \"$chn\" -j " + awgChain + " 2>/dev/null; do :; done\n")
	s.WriteString("  while ip6tables -w 5 -t mangle -D \"$chn\" -j " + awgChain + "6 2>/dev/null; do :; done\n")
	s.WriteString("done\n")
